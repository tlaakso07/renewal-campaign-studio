...
export function storedFiles(): string[] {
  ...
  for (const row of db
    .prepare("SELECT company,path,preview FROM assets")
    .all() as any[]) {
    for (const path of [row.path, row.preview])
      if (path) paths.add(`objects/${row.company}/${path}`);
  }
  for (const row of db
    .prepare("SELECT company,output FROM jobs WHERE output IS NOT NULL")
    .all() as any[]) {
    const out = JSON.parse(row.output);
    for (const key of ["file", "copyFile", "manifestFile", "captionsFile"])
      if (out[key]) paths.add(`objects/${row.company}/${out[key]}`);
  }
  for (const row of db
    .prepare(
      "SELECT body FROM records WHERE kind IN ('publication','community-media')",
    )
    .all() as any[]) {
    const body = JSON.parse(row.body);
    for (const file of [body.publicFile, body.captionsFile])
      if (file) paths.add(`published/${file}`);
  }
  return [...paths];
}
export class ReviewStore {
  ...
  private cached = new Map<
    string,
    { key: string; modified: number; bytes: number }
  >();
  constructor(
    public transport = blobTransport,
    public key = process.env.REVIEW_STATE_KEY || "review/state-v1.json",
  ) {
  ...
  async load() {
    ...
    if (!source)
      throw new AppError(
        503,
        "The private workspace has not been initialized.",
      );
    const state = JSON.parse(source.bytes.toString()) as ReviewSnapshot;
    if (state.schema !== 1 || !state.database || !state.files)
      throw new Error("Invalid private workspace snapshot");
    this.state = state;
  ...
  private local(path: string) {
    ...
    if (!/^(objects\/[a-zA-Z0-9_-]+|published)\/[^/]+$/.test(name))
      throw new AppError(403, "Invalid private file path");
    return { full, name };
  }
  async ensure(path: string) {
    ...
    if (!ref) {
      ...
      throw new AppError(404, "Private file unavailable");
    }
    ...
    if (!source || hash(source.bytes) !== ref.checksum)
      throw new AppError(503, "Private file could not be verified. Try again.");
    ...
    for (const path of storedFiles()) {
      ...
      if (!existsSync(full)) {
        if (!this.state.files[name])
          throw new Error("Referenced private file is missing");
        continue;
      }
      const info = statSync(full),
        cached = this.cached.get(name);
      if (
        cached?.modified === info.mtimeMs &&
        cached.bytes === info.size &&
        this.state.files[name]?.key === cached.key
      )
        continue;
  ...
  async commit() {
    ...
    this.state.database = readFileSync(resolve(DATA, "studio.sqlite")).toString(
      "base64",
    );
    this.state.updated = new Date().toISOString();
    this.etag = await this.transport.write(
      this.key,
