...
async function initialize() {
  ...
  if (
    ...
    process.env.APP_ENV !== "hosted-review" ||
  ...
  process.env.RENDER_FALLBACK_FONT = resolve(
    process.cwd(),
    "vendor/fonts/Lato-Regular.ttf",
  );
  const require = createRequire(import.meta.url);
  process.env.FFMPEG_PATH = require("ffmpeg-static");
  process.env.FFPROBE_PATH = require("ffprobe-static").path;
  const [{ app }, { ReviewStore, serialQueue }, database, worker, classroom] =
    await Promise.all([
      import("../server/index.ts"),
      import("../server/review-store.ts"),
      import("../server/db.ts"),
      import("../server/worker.ts"),
      import("../server/classroom.ts"),
    ]);
  const store = new ReviewStore(),
    exclusive = serialQueue();
  const curriculumActor = {
    company: "renewal",
    user: "review-creator",
    role: "owner",
    staff: true,
    name: "Hosted curriculum migration",
  };
  ...
  async function processPending() {
    ...
    try {
      await exclusive(async () => {
        ...
        db.prepare(
          "UPDATE jobs SET status='queued' WHERE status='running' AND updated<?",
        ...
        const usage = database.db
          .prepare("SELECT * FROM usage WHERE job=?")
          .get(job.id) as any;
        const output = result.output ? JSON.parse(result.output) : null;
        const outputAssetId =
          job.kind === "intake"
            ? JSON.parse(job.payload).assetId
            : job.kind === "generation"
              ? output?.assetId
              : null;
        const asset = outputAssetId
          ? (database.db
              .prepare("SELECT * FROM assets WHERE company=? AND id=?")
              .get(job.company, outputAssetId) as any)
          : null;
        ...
        for (let retry = 0; retry < 3; retry++) {
          ...
          const member = database.db
            .prepare(
              "SELECT role FROM memberships WHERE company=? AND user=? AND revoked=0",
            )
            .get(job.company, job.user) as any;
          if (!member || !["owner", "creator"].includes(member.role)) return;
          database.db
            .prepare(
              "UPDATE jobs SET status=?,progress=?,error=?,output=?,updated=? WHERE id=?",
            )
            .run(
              result.status,
              result.progress,
          ...
          if (usage)
            database.db
              .prepare(
                "UPDATE usage SET reserved=?,actual=?,state=? WHERE job=?",
              )
              .run(usage.reserved, usage.actual, usage.state, job.id);
          if (asset)
            database.db
              .prepare(
                `INSERT INTO assets(id,company,source_id,name,kind,status,source_url,collection,checksum,path,preview,bytes,metadata,error)
                 VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                 ON CONFLICT(company,id) DO UPDATE SET
                   name=excluded.name,kind=excluded.kind,status=excluded.status,
                   source_url=excluded.source_url,collection=excluded.collection,
                   checksum=excluded.checksum,path=excluded.path,preview=excluded.preview,
                   bytes=excluded.bytes,metadata=excluded.metadata,error=excluded.error`,
              )
              .run(
                asset.id,
                job.company,
          ...
          try {
            ...
            if (
              !(e instanceof database.AppError) ||
              e.status !== 409 ||
              retry === 2
            )
              throw e;
      ...
      console.error(
        "Private review worker did not commit",
        e instanceof database.AppError ? e.status : "storage-or-render-failure",
      );
    } finally {
      processing = false;
    }
...
export default async function handler(
  req: IncomingMessage & { reviewAuthorized?: boolean },
  res: ServerResponse,
) {
  try {
        ...
        if (mutation) {
          ...
          res.end = ((...args: any[]) => {
            ...
            (async () => {
              ...
              res.statusCode =
                e instanceof rt.database.AppError ? e.status : 503;
              res.setHeader("Content-Type", "application/json");
              end(
                JSON.stringify({
                  error:
                    res.statusCode === 409
                      ? "The workspace changed while saving. Reload and try again."
                      : "Your change was not saved. Storage is unavailable; try again.",
                }),
              );
            });
            return res;
    ...
    if (
      pathname === "/api/jobs" ||
      (mutation && pathname.startsWith("/api/jobs/"))
    )
      waitUntil(rt.processPending());
  } catch {
    if (res.headersSent) {
      res.destroy();
      return;
    ...
    res.setHeader("Content-Type", "application/json");
    res.end(
      '{"error":"Private workspace temporarily unavailable. Please try again."}',
    );
  }
}
