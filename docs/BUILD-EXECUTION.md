...
| 2 — local reconciliation complete; live source open                        | Measurement and creative decisions | CSV column mapping; ad performance detail/comparison; saved views; evidence-based review and next variation                                                                       | A14, A16, A18         |
| 3 — storyboard/audio complete; Seedance adapter wired, live output pending | Video and UGC                      | Run the first explicitly approved Seedance API generation, validate durable output, then use that generated asset in a branded timeline                                           | A07–A08, A11          |
| 4 — local product controls delivered; legacy caption inventory open        | Community and Classroom            | Complete threads/moderation, useful company directory and events; add eligible original recordings to the working Classroom player/resources/archives                             | A19–A21               |
...
**Generated-media checkpoint:** GPT-Image 2.5 Sunburst/Flare and all five verified Seedance 2.5 workflows are now wired through persistent private jobs. The AI Models page accepts validated prompts/references/settings, requires explicit provider-billing confirmation, records the exact endpoint/request lineage and copies completed media into tenant-scoped My Assets before reporting readiness. Generated Seedance clips can be selected in Video & UGC. The Higgsfield credential is configured locally and in Vercel without entering source control. Automated provider doubles prove durable storage and provenance without a paid request; connected Chrome verified the configured Seedance controls and the disabled local Gateway state. No live paid generation has been run, so A08 remains development/integration complete but externally unproven. Higgsfield API balance is separate from the website Unlimited plan; AI Gateway still needs purchased credits.

**Current local checkpoint:** accessibility/responsive shell work, report correction reconciliation, timed-caption enforcement and the available-video caption inventory are delivered. Build and 25 tests pass. Representative destinations reflow at 320 pixels; generated captions load in browser players; audio-bearing Community video now requires validated WebVTT; Classroom can store/inherit captions; unmuted source audio requires a scene transcript. Remaining local release checks are direct-upload browser submission, assistive-technology/200% zoom and a true OS-level render interruption. Thirty inaccessible video links join the wider source-original external dependency below.
