...
**Latest local Community delivery:** opt-in member profiles and directory, explainable contribution counts, shared/company events, published-ad attachments, post following, handle mentions and persisted user-scoped notifications now work through the real API. Authors can version-edit/remove/restore posts; staff moderation locks author restoration, reports carry reasons and resolve through auditable remove/dismiss actions, and notification preferences control follow, mention and eligible published-event notices. Public directory responses omit email and private home-company fields; company events remain tenant-scoped; attachments accept only explicitly published derivatives.

**Evidence:** production build and all **23 tests pass**. Isolated two-company Chrome walkthroughs verified profile opt-in, directory discovery under both themes, a shared event in local time, cross-company follows/mentions, post edit/remove/restore, reasoned reporting and operator dismissal to an empty queue. Automated coverage additionally proves moderator-locked restoration and company-event notification isolation. Temporary QA databases/servers never changed the main workspace.

**Remaining:** Astra inference is externally blocked by a Vercel 403 requiring paid AI Gateway credits. Adding a team payment method was not enough: the follow-up smoke test reported that this model is unavailable to free-tier users. The explicit fallback worked, so no successful model answer is claimed. The owned recording has an exact transcript and on-video instruction overlays but no timed caption track. Other media models remain disabled. Community still needs direct media uploads/richer composition and broader mobile/accessibility coverage.
...
| Community        | Persistent versioned posts/comments/polls/reactions; reasoned moderation; opt-in directory; scoped events; safe attachments; follows, mentions and preference-aware notifications | Direct media uploads/richer composition and broader mobile/accessibility coverage remain                                                  |
