...
**Latest local Community delivery:** opt-in member profiles and directory, explainable contribution counts, shared/company events, published-ad attachments, post following, handle mentions and persisted user-scoped notifications now work through the real API. Public directory responses omit email and private home-company fields; company events remain tenant-scoped; attachments accept only explicitly published derivatives.

**Evidence:** production build and all **23 tests pass**. An isolated two-company Chrome walkthrough verified profile opt-in, directory discovery under both interface themes, a shared event in local time, a cross-company discussion follow and mention, and separate follow/mention notifications for the correct Renewal user. The temporary QA database/server were removed from service without changing the main workspace. Earlier hosted evidence still verifies the eight-guide curriculum, owned H.264/PDF, playback/download/resume and Gateway OIDC reachability.

**Remaining:** Astra inference is externally blocked by a Vercel 403 requiring paid AI Gateway credits. Adding a team payment method was not enough: the follow-up smoke test reported that this model is unavailable to free-tier users. The explicit fallback worked, so no successful model answer is claimed. The owned recording has an exact transcript and on-video instruction overlays but no timed caption track. Other media models remain disabled. Community still needs direct media uploads/richer composition, notification-preference UI, broader moderation operations and mobile/accessibility coverage.
...
**Next:** finish the remaining Community composition/moderation/preferences work, then continue the broader video/mobile/accessibility walkthrough and reporting reconciliation. Keep untracked review-guide artifacts separate.
...
| Community        | Persistent posts/comments/polls/reactions, audiences/private drafts, reports/removal; opt-in directory, scoped events, published-ad attachments, follows, mentions and notifications | Direct media uploads/richer composition, notification-preference UI, complete moderation and broader mobile/accessibility coverage remain |
