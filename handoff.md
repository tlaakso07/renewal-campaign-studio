...
**Remaining:** Astra inference is externally blocked by a Vercel 403 requiring paid AI Gateway credits. Adding a team payment method was not enough: the follow-up smoke test reported that this model is unavailable to free-tier users. The explicit fallback worked, so no successful model answer is claimed. The owned recording has an exact transcript and on-video instruction overlays but no timed caption track. Other media models remain disabled. Community needs the broader mobile/keyboard/accessibility walkthrough; direct upload UI still needs a connected browser submission using synthetic media.
...
**Next:** perform the broader mobile/keyboard/accessibility walkthrough, then continue reporting reconciliation. Keep untracked review-guide artifacts separate.
