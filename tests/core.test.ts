...
const packet = (scenes: any[]) => ({
  ...
  avatar: { name: "Diane", role: "Homeowner", ageRange: "63-68", locale: "Kentucky suburban homeowner", wardrobe: "cozy cardigan", energy: ["plainspoken", "unhurried", "quietly relieved"], searchRef: "warm retired homeowner testimonial" }, scenes,
...
test("Editor Checklist: the agency's Concept 1 fails on fit and region; nothing else", async () => {
  ...
  assert.deepEqual(Object.keys(failed).map(Number), [3, 9], JSON.stringify(failed));
  ...
  assert.equal(failed[11], undefined, "S5 'like it always belonged' is the agency's own line, not a claim extension");
...
test("Brief writer: one structured call, computed numbers, all 16 checks pass; a client edit that no longer fits is flagged, not resized", async () => {
  ...
  const draft = async (system: string, prompt: string) => {
    ...
    return packet([
      scene(4, "Every October, this window let the cold right in.", "Older woman in a cozy cardigan at a frosted window, warm interior lamp light, photoreal, medium shot.", "slow push in", "50mm", "medium", "Hook. Hold long enough to read her breath on the glass."),
      ...
      scene(4, "They matched every frame like it had belonged.", "Installer fitting a white window frame into a Kentucky home exterior, autumn trees, daylight, photoreal, medium shot.", "slow orbit", "35mm", "medium", "Key proof-point scene. Show the frame flush against the trim."),
  ...
  assert.equal(plan.checks.length, 16);
